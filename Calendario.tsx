      ...
      days.push(
        <motion.div 
          ...
          className={`text-center py-2 rounded-full cursor-pointer transition-all duration-300 mx-1
                     ...
                     ${!isToday && !isSelected && hasEvents ? 'border border-primary-purple' : ''}
                     ${!isToday && !isSelected && !hasEvents ? 'hover:bg-background-purple/30' : ''}`}
          onClick={() => setSelectedDate(date)}
        >
          {day}
          {hasEvents && !isSelected && !isToday && (
            <div className="w-1 h-1 bg-primary-purple rounded-full mx-auto mt-1"></div>
          )}
        </motion.div>
      );
    }
    ...
    >
      <div className="container mx-auto px-4 md:px-6">
        <motion.div variants={itemVariants} className="text-center mb-12">
          <h1 className="text-4xl md:text-5xl font-bold mb-4">Agenda <span className="text-primary-purple">Lendária</span></h1>
          ...
          <motion.div variants={itemVariants} className="lg:col-span-2">
            <div className="bg-background-purple/30 p-6 rounded-lg shadow-lg h-full">
              <div className="flex items-center gap-3 mb-6">
                <Calendar size={24} className="text-primary-purple" />
                <h3 className="text-xl font-bold">
                ...
                <div className="space-y-4">
                  {eventsForSelectedDate.map(event => (
                    <motion.div 
                      key={event.id}
                      whileHover={{ y: -5 }}
                      className="bg-background-purple/50 p-4 rounded-lg border-l-4 border-primary-purple selection-line-animated"
                    >
                      <div className="flex justify-between items-start">
                        ...
                        <span className="bg-primary-purple px-3 py-1 rounded-full text-sm font-medium">
                          {event.type}
                        </span>
                      </div>
        ...
        <motion.div variants={itemVariants} className="mt-16 text-center">
          <h3 className="text-2xl font-bold mb-4">Próximos <span className="text-primary-purple">Eventos Especiais</span></h3>
          <div className="w-24 h-1 bg-primary-purple mx-auto mb-8"></div>
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
            {events.slice(0, 3).map(event => (
              ...
              >
                <div className="bg-primary-purple w-12 h-12 rounded-full flex items-center justify-center mb-4 mx-auto">
                  <Calendar size={24} />
                </div>
